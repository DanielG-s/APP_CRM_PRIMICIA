...
export class ProductSyncService {
    ...
    async handleDailySync() {
        ...
        const delayStepMs = Number(this.configService.get('ERP_SYNC_DELAY_PRODUCTS_MS_STEP', 300000)); // Default 5 mins
        const maxDelayMs = Number(this.configService.get('ERP_SYNC_DELAY_PRODUCTS_MS_MAX', 7200000)); // Default 2 hours cap
        ...
        while (hasMore) {
            ...
            for (const store of stores) {
                ...
                this.logger.log(`Queueing Daily Product Sync for ${store.name} (Tenant: ${tenantId}) with ${delay / 1000}s delay. JobId: ${customJobId}`);

                await this.syncQueue.add('sync-products', { tenantId }, {
                    jobId: customJobId,
                    delay: delay,
                    attempts: 3,
    ...
    async syncProducts() {
        try {
            ...
            while (hasMore) {
                const products = await this.milleniumService.getProductsPaginated(batchSize, skip);
                ...
                this.logger.log(`Processing batch of ${products.length} products (Skip: ${skip})...`);
                ...
                const operations = products.map(async (raw) => {
                    ...
                    const productData = {
                        ...
                        category: raw.departamento !== undefined && raw.departamento !== null ? String(raw.departamento) : null,
                        brand: raw.marca !== undefined && raw.marca !== null ? String(raw.marca) : null,
                        status: 'ACTIVE',
                        updatedAt: new Date()
                    };
                ...
                if (updatedCount >= 1000000) {
                    this.logger.warn('Safety limit reached (1,000,000 products). Stopping sync.');
                    break;
                }
            }

            this.logger.log(`Product Sync Finished. Processed ${updatedCount} products.`);
