  ...
  async syncStores() {
    try {
      ...
      for (const raw of filiais) {
        ...
        const isExcluded =
          ['DESAT', 'DIV-', '10', 'FC', 'INDEF', 'TRE'].some((prefix) =>
            codFilial.startsWith(prefix),
          ) || ['001', '002', '003'].includes(codFilial);
        if (isExcluded) {
          this.logger.debug(
            `Ignoring store ${raw.nome} due to prefix: ${codFilial}`,
          );
          continue;
        }
      ...
      this.logger.log(
        `Store Sync Finished. Processed and active stores updated: ${updatedCount}`,
      );
    } catch (error) {
