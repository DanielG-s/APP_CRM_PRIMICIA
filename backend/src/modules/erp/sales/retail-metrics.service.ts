...
export class RetailMetricsService {
    ...
    async getMetrics(start: Date, end: Date, storeIds?: string[]) {
        ...
        const recurringCustomers = customersWithCounts.filter(c => c._count.id > 1).length;
        const repurchaseRate = activeCustomers > 0 ? (recurringCustomers / activeCustomers) * 100 : 0;
