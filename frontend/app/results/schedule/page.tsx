...
export default function AgendaPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:3000/sales/filter-options').then(res=>res.json()).then(data => {
        if(data) {
            setOptionsData({ 
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3000/sales/schedule-metrics?${params.toString()}`);
