...
export const StatCard = ({ icon: Icon, label, value, color = "blue" }: any) => {
  ...
  return (
    <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-start gap-3">
      ...
      <div><p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{label}</p><p className="text-lg font-bold text-slate-900 mt-0.5">{value}</p></div>
    </div>
  );
};
  ...
  return (
    <div className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-lg transition-colors">
      ...
      <div className="flex-1">
        <div className="flex justify-between items-center mb-1"><span className="text-sm font-medium text-slate-700">{label}</span><span className="text-xs font-bold text-slate-900">{count} ({percent}%)</span></div>
        <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${color.replace('text-', 'bg-').replace('500', '500')}`} style={{ width: `${percent}%` }}></div></div>
      </div>
    </div>
  );
};
...
export const LogicConnector = ({ logicOperator, toggleLogic }: any) => (
  <div className="absolute -top-7 left-1/2 transform -translate-x-1/2 z-10 flex flex-col items-center group/connector">
    <div className="h-4 w-0.5 bg-gray-200 absolute -top-4"></div><div className="h-4 w-0.5 bg-gray-200 absolute -bottom-4"></div>
    <button onClick={toggleLogic} className={`relative z-20 px-4 py-1.5 rounded-full text-xs font-bold border-2 shadow-sm transition-all uppercase tracking-wider cursor-pointer flex items-center gap-1.5 hover:scale-105 active:scale-95 ${logicOperator === 'OR' ? 'bg-orange-50 text-orange-600 border-orange-200' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>{logicOperator === 'OR' ? <Split size={12} /> : <ListFilter size={12} />} {logicOperator === 'OR' ? 'OU' : 'E'}</button>
  </div>
);
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 relative">
          <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-slate-50/50"><h3 className="text-lg font-bold text-slate-800">Escolha o tipo de regra</h3><button onClick={onClose} className="p-2 hover:bg-slate-200 rounded-full text-slate-500 transition-colors cursor-pointer"><X size={20} /></button></div>
          <div className="p-6 grid gap-4 max-h-[70vh] overflow-y-auto">
            <button onClick={() => onSelect('rfm')} className="flex items-center gap-4 p-4 rounded-xl border-2 border-slate-100 hover:border-amber-500 hover:bg-amber-50 transition-all group text-left cursor-pointer"><div className="w-12 h-12 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform"><Gauge size={24} /></div><div><span className="block font-bold text-slate-800 text-lg">Ciclo de Vida (RFM)</span><span className="text-sm text-slate-500">Filtrar por status (ex: Champions, Em Risco)</span></div></button>
            <button onClick={() => onSelect('behavioral')} className="flex items-center gap-4 p-4 rounded-xl border-2 border-slate-100 hover:border-blue-500 hover:bg-blue-50 transition-all group text-left cursor-pointer"><div className="w-12 h-12 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform"><Activity size={24} /></div><div><span className="block font-bold text-slate-800 text-lg">Comportamento</span><span className="text-sm text-slate-500">Baseado em ações (ex: Comprou, Visitou)</span></div></button>
            <button onClick={() => onSelect('characteristic')} className="flex items-center gap-4 p-4 rounded-xl border-2 border-slate-100 hover:border-purple-500 hover:bg-purple-50 transition-all group text-left cursor-pointer"><div className="w-12 h-12 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform"><UserCog size={24} /></div><div><span className="block font-bold text-slate-800 text-lg">Característica</span><span className="text-sm text-slate-500">Dados do perfil (ex: Cidade, Gênero)</span></div></button>
            <button onClick={() => onSelect('segment_ref')} className="flex items-center gap-4 p-4 rounded-xl border-2 border-slate-100 hover:border-teal-500 hover:bg-teal-50 transition-all group text-left cursor-pointer"><div className="w-12 h-12 rounded-full bg-teal-100 text-teal-600 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform"><PieChart size={24} /></div><div><span className="block font-bold text-slate-800 text-lg">Segmento</span><span className="text-sm text-slate-500">Reutilizar um grupo já existente</span></div></button>
          </div>
        </div>
      </div>
    );
...
export const RfmBlock = ({ block, onUpdate, onRemove }: any) => {
    return (<div className="p-6"><div className="flex items-center gap-3 mb-5 pb-4 border-b border-amber-100/50"><div className="bg-amber-100 p-2 rounded-lg text-amber-700 shadow-sm"><Gauge size={20} /></div><div><span className="block text-xs font-bold text-amber-600 uppercase tracking-widest">Ciclo de Vida (RFM)</span></div><button onClick={() => onRemove(block.id)} className={BTN_ICON_CLASS}><Trash2 size={18} /></button></div><div className="flex flex-col md:flex-row items-center gap-4 bg-amber-50/50 p-4 rounded-xl border border-amber-100"><div className="flex-1 w-full relative"><label className="text-xs font-bold text-amber-800 uppercase mb-1 block ml-1">Status do Cliente</label><div className="relative"><select className={`${SELECT_CLASS} pl-3 border-amber-200 focus:border-amber-400 focus:ring-amber-500 h-[48px] font-semibold text-gray-800 bg-white`} value={block.status} onChange={(e) => onUpdate({ ...block, status: e.target.value })}><option value="" disabled>Selecione um status...</option>{RFM_STATUSES.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}</select></div>{block.status && <p className="text-xs text-amber-600/80 mt-2 ml-1">ℹ️ {RFM_STATUSES.find(s => s.id === block.status)?.desc}</p>}</div></div></div>);
};
...
export const BehaviorBlock = ({ block, onUpdate, onRemove, uiOptions }: any) => {
    ...
    return (<div className="p-6 relative"><div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6"><div className="flex bg-slate-100 p-1 rounded-xl shrink-0"><button onClick={() => onUpdate({ ...block, type: 'did' })} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all ${block.type === 'did' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}><CheckCircle2 size={14} /> Fizeram</button><button onClick={() => onUpdate({ ...block, type: 'did_not' })} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all ${block.type === 'did_not' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}><XCircle size={14} /> Não fizeram</button></div><div className="relative flex-1 w-full sm:w-auto min-w-[200px]"><div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-blue-500"><Activity size={18} /></div><select className={`${SELECT_CLASS} pl-10 border-blue-100 focus:border-blue-300 focus:ring-blue-500 h-[40px] font-semibold text-gray-800`} value={block.event} onChange={(e) => onUpdate({ ...block, event: e.target.value, filters: [] })}><option value="" disabled>Selecione um evento...</option>{Object.keys(EVENT_CATEGORIES).map(catKey => (<optgroup key={catKey} label={EVENT_CATEGORIES[catKey].label}>{EVENTS_DB.filter(e => e.category === catKey).map(ev => <option key={ev.id} value={ev.id}>{ev.label}</option>)}</optgroup>))}</select></div><button onClick={() => onRemove(block.id)} className={`${BTN_ICON_CLASS} shrink-0`} title="Remover regra"><Trash2 size={18} /></button></div>{block.event && (<div className="ml-2 pl-4 border-l-2 border-slate-100 space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">{block.filters.length === 0 && <div className="text-xs text-slate-400 italic mb-2">Nenhum filtro aplicado (considerando todos).</div>}{block.filters.map((filter: any, idx: number) => { const selectedProp = propertiesAvailable.find(p => p.id === filter.field); const type = selectedProp?.type || 'string'; const ops = OPERATORS[type] || OPERATORS.string; const opDef = ops.find((o: any) => o.id === filter.operator); return (<div key={filter.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-2 p-2 bg-slate-50 rounded-lg border border-slate-100 group hover:border-blue-200 transition-colors"><div className="flex-1 w-full sm:w-auto flex gap-2"><select className={`${SELECT_CLASS} py-1.5 text-xs focus:ring-blue-500`} value={filter.field} onChange={(e) => handleUpdateFilter(idx, { ...filter, field: e.target.value, operator: '', value: '' })}><option value="" disabled>Propriedade</option>{propertiesAvailable.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}</select><select className={`${SELECT_CLASS} w-[110px] py-1.5 text-xs focus:ring-blue-500`} value={filter.operator} onChange={(e) => handleUpdateFilter(idx, { ...filter, operator: e.target.value })} disabled={!filter.field}><option value="" disabled>Operador</option>{ops.map((op: any) => <option key={op.id} value={op.id}>{op.label}</option>)}</select></div>{!opDef?.noValue && (<div className="w-full sm:w-auto sm:flex-1 sm:min-w-[150px]"><DynamicValueInput type={type} value={filter.value} options={selectedProp?.options} onChange={(e: any) => handleUpdateFilter(idx, { ...filter, value: e.target.value })} disabled={!filter.operator} /></div>)}<button onClick={() => onUpdate({ ...block, filters: block.filters.filter((_: any, i: number) => i !== idx) })} className="text-slate-300 hover:text-red-500 p-1.5 opacity-0 group-hover:opacity-100 transition-all cursor-pointer"><Trash2 size={14} /></button></div>); })}<button onClick={() => onUpdate({ ...block, filters: [...block.filters, { id: Date.now(), field: '', operator: '', value: '' }] })} className="group flex items-center gap-2 text-xs text-slate-500 hover:text-blue-600 font-bold px-3 py-2 rounded-lg hover:bg-blue-50 transition-all mt-2"><div className="bg-slate-200 group-hover:bg-blue-200 text-slate-500 group-hover:text-blue-600 rounded p-0.5 transition-colors"><Plus size={12} /></div> Adicionar filtro</button></div>)}</div>);
};
...
export const CharacteristicBlock = ({ block, onUpdate, onRemove, uiOptions }: any) => {
    ...
    return (<div className="p-6"><div className="flex items-center gap-3 mb-5 pb-4 border-b border-purple-100/50"><div className="bg-purple-100 p-2 rounded-lg text-purple-700 shadow-sm"><UserCog size={20} /></div><div><span className="block text-xs font-bold text-purple-600 uppercase tracking-widest">Característica</span></div><button onClick={() => onRemove(block.id)} className={BTN_ICON_CLASS}><Trash2 size={18} /></button></div><div className="flex flex-col md:flex-row items-center gap-3"><div className="w-full md:flex-1"><div className="relative"><select className={`${SELECT_CLASS} pl-9 border-purple-100 focus:border-purple-300 focus:ring-purple-500`} value={block.field} onChange={(e) => onUpdate({ ...block, field: e.target.value, operator: '', value: '' })}><option value="" disabled>Selecione um atributo...</option>{CUSTOMER_FIELDS_DYNAMIC.map(f => <option key={f.id} value={f.id}>{f.label}</option>)}</select><Fingerprint size={16} className="absolute left-3 top-3 text-purple-400 pointer-events-none" /></div></div><div className="w-full md:w-[180px]"><select className={`${SELECT_CLASS} border-purple-100 focus:border-purple-300 focus:ring-purple-500`} value={block.operator} onChange={(e) => onUpdate({ ...block, operator: e.target.value })} disabled={!block.field}><option value="" disabled>Operador</option>{operators.map((op: any) => <option key={op.id} value={op.id}>{op.label}</option>)}</select></div>{!currentOpDef?.noValue && (<div className="w-full md:flex-1"><DynamicValueInput type={inputType} value={block.value} options={selectedField?.options} onChange={(e: any) => onUpdate({ ...block, value: e.target.value })} disabled={!block.operator} placeholder="Valor..." /></div>)}</div></div>);
};

export const SegmentReferenceBlock = ({ block, onUpdate, onRemove, uiOptions }: any) => {
    return (<div className="p-6"><div className="flex items-center gap-3 mb-5 pb-4 border-b border-teal-100/50"><div className="bg-teal-100 p-2 rounded-lg text-teal-700 shadow-sm"><PieChart size={20} /></div><div><span className="block text-xs font-bold text-teal-600 uppercase tracking-widest">Segmento</span></div><button onClick={() => onRemove(block.id)} className={BTN_ICON_CLASS}><Trash2 size={18} /></button></div><div className="flex flex-col md:flex-row items-center gap-4 bg-teal-50/50 p-4 rounded-xl border border-teal-100"><div className="flex bg-white p-1 rounded-lg shrink-0 shadow-sm w-full md:w-auto"><button onClick={() => onUpdate({ ...block, relation: 'in' })} className={`flex-1 md:flex-none justify-center flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold transition-all ${block.relation === 'in' ? 'bg-teal-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}><CheckCircle2 size={16} /> Faz parte</button><button onClick={() => onUpdate({ ...block, relation: 'not_in' })} className={`flex-1 md:flex-none justify-center flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold transition-all ${block.relation === 'not_in' ? 'bg-rose-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}><XCircle size={16} /> Não faz parte</button></div><div className="flex-1 w-full relative"><Users size={18} className="absolute left-3 top-3 text-teal-500 pointer-events-none" /><select className={`${SELECT_CLASS} pl-10 border-teal-200 focus:border-teal-400 focus:ring-teal-500 h-[42px] font-semibold text-gray-700 bg-white`} value={block.segment_id} onChange={(e) => onUpdate({ ...block, segment_id: e.target.value })}><option value="" disabled>Selecione um segmento...</option>{uiOptions.segments && uiOptions.segments.map((seg: any) => <option key={seg.id} value={seg.id}>{seg.name}</option>)}</select></div></div></div>);
};