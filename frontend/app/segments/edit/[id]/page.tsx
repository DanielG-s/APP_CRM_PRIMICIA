...
import { 
  ...
  AlertCircle, Save, X, RefreshCcw, Lock, Gauge, Info,
  BookOpen, Lightbulb, ArrowLeft, MousePointerClick, Zap // <--- Ícones Importados
} from 'lucide-react';
...
const UI_OPTIONS_INITIAL = {
  ...
  dates: ['2024-01-01', 'Hoje', 'Ontem', 'Últimos 7 dias', 'Últimos 30 dias', 'Últimos 60 dias', 'Últimos 90 dias'],
...
// --- NOVO COMPONENTE: MODAL DE TUTORIAL (EDIÇÃO) ---
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] animate-in zoom-in-95 duration-300">
        ...
        <div className="bg-slate-900 text-white p-8 md:w-1/3 flex flex-col justify-between relative overflow-hidden">
          <div className="relative z-10">
            ...
            <h2 className="text-2xl font-bold mb-2">Editando<br/>Segmentos</h2>
            <p className="text-slate-400 text-sm leading-relaxed">
              Dicas para refinar sua estratégia e ajustar suas audiências em tempo real.
            </p>
          </div>
          
          <div className="relative z-10 mt-8 space-y-4">
            <div className="flex items-center gap-3 text-sm font-medium text-slate-300">
              <div className="w-2 h-2 rounded-full bg-emerald-500"></div> Ajuste Fino
            </div>
            <div className="flex items-center gap-3 text-sm font-medium text-slate-300">
              <div className="w-2 h-2 rounded-full bg-emerald-500"></div> Preview Instantâneo
            </div>
            <div className="flex items-center gap-3 text-sm font-medium text-slate-300">
              <div className="w-2 h-2 rounded-full bg-emerald-500"></div> Teste A/B Mental
            </div>
          </div>
        ...
        <div className="flex-1 bg-white p-8 overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              <Lightbulb size={20} className="text-amber-500" /> O que você pode fazer?
            </h3>
            <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors cursor-pointer"><X size={24} /></button>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="flex gap-4">
              ...
              <div>
                <h4 className="font-bold text-slate-900 mb-1 flex items-center gap-2">Adicionar ou Remover Regras</h4>
                <p className="text-sm text-slate-500 mb-2">
                  Percebeu que o público ficou pequeno demais? Tente remover uma regra clicando na lixeira <Trash2 size={12} className="inline"/> no canto do bloco.
                </p>
                <p className="text-sm text-slate-500">
                  Quer ser mais específico? Adicione uma nova regra clicando em "Nova Regra" lá embaixo.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <div>
                <h4 className="font-bold text-slate-900 mb-1 flex items-center gap-2">Olho no Impacto <BarChart3 size={16} className="text-slate-400"/></h4>
                <p className="text-sm text-slate-500">
                  Sempre que você altera algo, o painel à direita recalcula o <strong>Alcance</strong> e a <strong>Receita Estimada</strong>. Use isso para encontrar o equilíbrio ideal entre quantidade e qualidade.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <div>
                <h4 className="font-bold text-slate-900 mb-1 flex items-center gap-2">Mudar para Estático? <Lock size={16} className="text-slate-400"/></h4>
                <p className="text-sm text-slate-500">
                  Se você vai fazer uma campanha pontual (ex: Black Friday) e não quer que a lista mude no meio do caminho, mude a chave no topo para <strong>Estático</strong>.
                </p>
              </div>
            </div>
...
// --- SUB-COMPONENTES (Blocos, Stats, etc) ---
...
const BehaviorBlock = ({ block, onUpdate, onRemove, uiOptions }: any) => {
  const eventData = getEventById(block.event);
  const propertiesAvailable = useMemo(() => {
      const event = EVENTS_DB.find(e => e.id === block.event);
      if (!event) return [];
      
      const definitions: any = {
        _STANDARD: [ { id: 'date', label: 'Data', type: 'date', options: uiOptions.dates }, { id: 'count', label: 'Frequência', type: 'number' } ],
        NAV: [ { id: 'device', label: 'Dispositivo', type: 'string', options: ['mobile', 'desktop'] }, { id: 'nome_categoria', label: 'Nome da Categoria', type: 'string', options: uiOptions.categories }, { id: 'nome_departamento', label: 'Nome do Departamento', type: 'string', options: uiOptions.departments } ],
        TRANSACTION: [ { id: 'produto_id', label: 'Nome do Produto', type: 'string', options: uiOptions.products }, { id: 'categoria', label: 'Categoria', type: 'string', options: uiOptions.categories }, { id: 'subtotal', label: 'Valor (R$)', type: 'number' }, { id: 'total', label: 'Total Pedido (R$)', type: 'number' }, { id: 'quantidade_produtos', label: 'Qtd. Produtos', type: 'number' } ],
        COMMUNICATION: [ { id: 'campaign_id', label: 'ID da Campanha', type: 'string', options: uiOptions.campaigns }, { id: 'subject', label: 'Assunto', type: 'string', options: uiOptions.search_terms } ],
        SEARCH: [ { id: 'termo_busca', label: 'Termo buscado', type: 'string', options: uiOptions.search_terms }, { id: 'total_resultados', label: 'Total de Resultados', type: 'number' } ]
      };
    ...
    <div className="p-6 relative">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        ...
        <div className="relative flex-1 w-full sm:w-auto min-w-[200px]">
          ...
          <select className={`${SELECT_CLASS} pl-10 border-blue-100 focus:border-blue-300 focus:ring-blue-500 h-[40px] font-semibold text-gray-800`} value={block.event} onChange={(e) => onUpdate({ ...block, event: e.target.value, filters: [] })}>
        ...
        <button onClick={() => onRemove(block.id)} className={`${BTN_ICON_CLASS} shrink-0`} title="Remover regra"><Trash2 size={18} /></button>
      </div>
      {block.event && (
        <div className="ml-2 pl-4 border-l-2 border-slate-100 space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
            {block.filters.length === 0 && <div className="text-xs text-slate-400 italic mb-2">Nenhum filtro aplicado (considerando todos).</div>}
          ...
          <button onClick={() => onUpdate({ ...block, filters: [...block.filters, { id: Date.now(), field: '', operator: '', value: '' }] })} className="group flex items-center gap-2 text-xs text-slate-500 hover:text-blue-600 font-bold px-3 py-2 rounded-lg hover:bg-blue-50 transition-all mt-2">
            <div className="bg-slate-200 group-hover:bg-blue-200 text-slate-500 group-hover:text-blue-600 rounded p-0.5 transition-colors"><Plus size={12} /></div> Adicionar filtro
          </button>
        </div>
      )}
    </div>
...
export default function EditSegmentPage() {
  ...
  // CARREGAR DADOS DO SEGMENTO E FILTROS
  useEffect(() => {
    async function fetchData() {
        try {
            // 1. Filtros
            const resFilters = await fetch('http://localhost:3000/webhook/crm/intelligence/filters');
            if (resFilters.ok) {
            ...
            // 2. Segmento
            if (segmentId) {
                const resSegment = await fetch(`http://localhost:3000/webhook/crm/intelligence/segments/${segmentId}`);
                if (resSegment.ok) {
                    const segment = await resSegment.json();
                    setSegmentName(segment.name);
                    setIsDynamic(segment.isDynamic);
                    if (Array.isArray(segment.rules)) {
                        setBlocks(segment.rules);
                    }
                } else {
  ...
  const handleSave = async () => {
    if (!segmentName.trim()) { alert('Por favor, dê um nome ao segmento.'); return; }
    ...
    if (hasInvalidBlock) { alert('Existem regras incompletas.'); return; }

    setIsSaving(true); setSaveStatus('saving');
    try {
      const response = await fetch(`http://localhost:3000/webhook/crm/intelligence/segments/${segmentId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (isLoadingData) {
      return <div className="flex h-screen items-center justify-center bg-slate-50 text-slate-400">Carregando segmento...</div>;
  }
